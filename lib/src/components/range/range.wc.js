...
class Range extends LitElement {
  constructor() {
    ...
    this._handleAddPoint = this._handleAddPoint.bind(this);
  ...
  _handleKeyDown(i, e) {
    ...
    if (e.keyCode === 8 || e.keyCode === 46) {
      if (this.editable) {
        this.value = this.arrayValues.filter((v, index) => i !== index);
      }
    }
    if (e.keyCode === 39) {
  ...
  _handleAddPoint(e) {
    const isThumb = e.target.getAttribute("part") === "thumb";
    if (this.editable && !isThumb) {
      const lineStart = this.getBoundingClientRect().left;
      const lineLength = this.clientWidth;
      ...
      const valueByNumber = this.range * (valueByPercent / 100);
      const newValue = Math.round(valueByNumber / this.step) * this.step;
      this.value = [...this.arrayValues, newValue];
    }
  }
  ...
  render() {
    return html`
      <div part="line-wrapper">
        <div
          part="line"
          @mousedown=${this._handleAddPoint}
          @touchstart=${this._handleAddPoint}
        >
